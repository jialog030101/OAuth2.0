...
public class AppServiceImpl extends BaseServiceImpl<AppMapper, App> implements AppService, AppManager {
	...
	public Result<Long> validate(String clientId) {
		   // TODO: 请同学们在此处实现校验应用ID的逻辑
        //
        // 1. 根据 clientId 从数据库中查找应用信息
        //    提示: 可以使用 selectByClientId(clientId) 方法
        //
        // 2. 如果应用不存在，返回 Result.error("非法应用")
        //
        // 3. 如果应用存在，返回 Result.success(app.getId())
	...
	public Result<Void> validate(String clientId, String clientSecret) {
		 // TODO: 请同学们在此处实现校验应用ID和密钥的逻辑
        //
        // 1. 根据 clientId 从数据库中查找应用信息
        //    提示: 可以使用 selectByClientId(clientId) 方法
        //
        // 2. 如果应用不存在，返回 Result.error("clientId不存在")
        //
        // 3. 校验应用密钥(clientSecret)是否匹配
        //    如果不匹配，返回 Result.error("appSecret有误")
        //
        // 4. 验证通过，返回 Result.success()
